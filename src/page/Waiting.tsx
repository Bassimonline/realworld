...
const StepOnePurchase = ({ 
  ...
  return (
    <motion.div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8" {...fadeInUp}>
      ...
      {/* LEFT COLUMN: Value Props / Instructions */}
...
const StepTwoPayment = ({ 
  ...
  const qrData = `solana:${WALLET_ADDRESS}?amount=${details.solAmount}`
      ...
      <div className="bg-[#121214] border border-white/10 rounded-xl overflow-hidden shadow-2xl"> 
        ...
        <div className="flex border-b border-white/10 bg-black/20">
          <button 
            ...
            // FIX 2: Better visibility for inactive text
            className={`flex-1 py-2 text-sm font-semibold uppercase tracking-wider transition-all relative ${activeTab === "wallet" ? "bg-yellow-500 text-black" : "text-gray-300 hover:text-white bg-transparent"}`}
          >
            <span className="flex items-center justify-center gap-2"><Wallet size={16} /> Crypto Wallet</span>
            {activeTab === "wallet" && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-yellow-500" />}
          </button>
          <button 
            onClick={() => setActiveTab("qr")}
            // FIX 2: Better visibility for inactive text
            className={`flex-1 py-2 text-sm font-semibold uppercase tracking-wider transition-all relative ${activeTab === "qr" ? "bg-yellow-500 text-black" : "text-gray-300 hover:text-white bg-transparent"}`}
          >
            <span className="flex items-center justify-center gap-2"><QrCode size={16} /> QR Code</span>
            {activeTab === "qr" && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-yellow-500" />}
          </button>
        </div>
          ...
          {activeTab === "wallet" ? (
            <div className="space-y-6"> 
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center justify-between bg-black/40 border border-white/10 rounded-lg p-3 group"> 
                  <div
                    className="overflow-hidden mr-4 cursor-pointer"
                    role="button"
                    tabIndex={0}
                    title="Click to copy address"
                    onClick={() => copyToClipboard(WALLET_ADDRESS, "address")}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); copyToClipboard(WALLET_ADDRESS, 'address') } }}
                  >
                  ...
                  <div className="shrink-0">
                    {copiedField === "address" ? <CheckCircle size={16} className="text-green-500" /> : null}
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center justify-center py-6 animate-in fade-in">
              ...
              <p className="text-white font-bold text-base mt-4">Scan to Pay</p>
              <p className="text-gray-500 text-sm mt-1">Send exactly <span className="text-yellow-500 font-bold">{details.solAmount} SOL</span></p>
            </div>
          )}
        </div>
