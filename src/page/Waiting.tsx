...
const WALLET_ADDRESS = "Hgrhwnue9nsBTRfy6S6xtqNVeQjiDUByTaT6yxYgYwaL"
...
// Animation for the amount box container
const amountBoxVariants = {
...
// Keyframes for the "snake line" border effect
const snakeLineAnimation = {
...
const StepTwoTransfer = ({ details, onNext }: { details: any, onNext: () => void }) => {
    ...
    // Remove commas for accurate display
    const solAmountDisplay = details.solAmount ? String(details.solAmount).replace(/,/g, "") : "N/A";
    
    // Total tokens display is no longer used in the main instruction text per request.
    // const totalTokensDisplay = details.totalTokens ? details.totalTokens.toLocaleString() : "N/A";

    const paymentSteps = [
        "Send the **exact amount** of SOL displayed below from your wallet.",
            ...
            <div className="bg-[#121214] border border-white/10 rounded-xl p-6 shadow-2xl relative">
                ...
                <div className="text-center mb-6">
                    ...
                    <div className="flex justify-center">
                         <motion.div 
                            // Change from inline-flex to a block with max-width and margin for centering
                            className="relative w-full max-w-sm p-[3px] rounded-xl overflow-hidden shadow-2xl" 
                            variants={amountBoxVariants}
                            initial="initial"
                            animate="animate"
                            whileHover="hover"
                            style={{
                                // Define a repeating gradient background for the border effect
                                background: `linear-gradient(90deg, transparent 0%, transparent 20%, ${'#FCD34D'} 50%, transparent 80%, transparent 100%)`,
                                backgroundSize: '200% 100%', // Twice the width to allow animation
                            }}
                            // Apply the snake line animation directly to the background position
                            variants={snakeLineAnimation}
                            initial="initial"
                            animate="animate"
                        >
                            {/* Inner content div, using flex-1 to stretch the inner background to full width */}
                            <div className="relative z-10 flex items-center justify-center bg-[#18181b] rounded-xl px-8 py-4 w-full"> 
                                <span className="text-3xl sm:text-4xl font-extrabold text-white font-mono leading-none tracking-tight drop-shadow-[0_0_6px_rgba(252,211,77,0.7)]">
                                    {solAmountDisplay} 
                                </span>
                                <span className="text-xl sm:text-2xl ml-2 text-yellow-300 font-semibold">SOL</span>
                            </div>
                        </motion.div>
                    </div>
                </div>
                {/* --- MODIFIED TEXT/AMOUNT BLOCK END --- */}
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-lg p-4">
                        <h3 className="text-white font-bold text-sm tracking-wide uppercase mb-3 flex items-center gap-2">
                            <ListOrdered size={16} className="text-yellow-500" /> Payment Instructions
                        </h3>
                        <ul className="space-y-2">
                            <li className="flex items-start gap-2">
                                ...
                                <span className="text-gray-300 font-medium text-sm leading-snug">
                                    Send the **exact amount** shown above to the recipient address.
                                </span>
                            </li>
